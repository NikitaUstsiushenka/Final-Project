...
public abstract class AbstractSQLQuery {
    ...
    /**
     * Value of the sql query that updates drug count by id.
     */
    public static final String UPDATE_DRUG_COUNT;

    /**
     * Value of the sql query that updates drug count by name and dosage.
     */
    public static final String UPDATE_DRUG_COUNT_BY_DOSAGE;
    ...
    /**
     * Value of the sql query that selects order items by order item.
     */
    public static final String SELECT_ITEMS_BY_ORDER_ID;

    /**
     * Value of the sql query that updates order price.
     */
    public static final String UPDATE_PRICE;

    /**
     * Value of the sql query that updates order paid.
     */
    public static final String UPDATE_PAID;

    /**
     * Value of the sql query that selects all clients.
     */
    public static final String SELECT_USERS_BY_ROLE;

    /**
     * Value of the sql query that deletes user by id.
     */
    public static final String DELETE_CLIENT_BY_ID;

    /**
     * Value of the sql query that selects orders by client id.
     */
    public static final String SELECT_ORDERS_BY_ID;

    /**
     * Value of the sql query that selects user by id.
     */
    ...
    static {
        ...
        INSERT_ORDER_ITEM = "INSERT INTO `order_item` (`order_id`, "
                + "`drug_id`, `recipe_id`, `count`) VALUES((SELECT o.`id` "
                + "FROM `order` o WHERE o.`id` = ?), ?, (SELECT r.`id` FROM "
                + "`recipe` r WHERE r.`id` = ?), ?)";
        ...
        SELECT_ITEMS_BY_ORDER_ID = "SELECT * FROM `order_item` oi "
                + "WHERE oi.`order_id` = ?";
        UPDATE_PRICE = "UPDATE `order` o SET o.`price` = ? "
                + "WHERE o.`client_id` = ? AND o.`is_paid` = ?";
        UPDATE_PAID = "UPDATE `order` o SET o.`is_paid` = ? WHERE o.`id` = ?";
        SELECT_USERS_BY_ROLE = "SELECT * FROM `user` u WHERE u.`role` = ?";
        DELETE_CLIENT_BY_ID = "DELETE FROM `user` WHERE `id` = ?";
        SELECT_ORDERS_BY_ID = "SELECT * FROM `order` o "
                + "WHERE o.`client_id` = ?";
        UPDATE_DRUG_COUNT_BY_DOSAGE = "UPDATE `drug` d SET d.`count` = ? "
                + "WHERE d.`name` = ? AND d.`dosage` = ?";
        SELECT_USER_BY_ID = "SELECT * FROM `user` u WHERE u.`id` = ?";
